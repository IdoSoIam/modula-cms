<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-[130]">
      ...
      <div
        ref="panelRef"
        class="absolute w-[min(48rem,calc(100vw-1.5rem))] max-w-3xl overflow-hidden rounded-2xl border border-base-300 bg-base-100 shadow-2xl"
        :style="panelStyle"
      >
        <div
          class="flex cursor-move items-center justify-between gap-3 border-b border-base-300 bg-base-200 px-5 py-4 select-none"
...
const clampPosition = () => {
  ...
  panelPosition.value = {
    x: Math.min(Math.max(8, panelPosition.value.x), maxX),
    y: Math.min(Math.max(8, panelPosition.value.y), maxY)
  }
}
