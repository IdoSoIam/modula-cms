    ...
    <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
      <div :class="getColumnVerticalAlignClass()">
        <template v-if="section.column.type === 'image'">
          <div class="overflow-hidden rounded-[2rem] border border-base-300 bg-base-100 shadow-sm" :class="imageAspectClass(section.column.aspect)">
            <img
        ...
        <template v-else>
          <div class="space-y-5" :class="contentAlignClass(section.column.align)" :style="columnTextStyle(section.column)">
            <template v-if="hasColumnHeaderContent(section.column)">
              <div v-if="pickLocalizedText(locale, section.column.badge)" class="badge badge-primary badge-outline">
                {{ pickLocalizedText(locale, section.column.badge) }}
              </div>

              <h2 v-if="pickLocalizedText(locale, section.column.title)" class="text-3xl font-bold">
                {{ pickLocalizedText(locale, section.column.title) }}
              </h2>

              <p v-if="pickLocalizedText(locale, section.column.text)" class="text-base opacity-80 md:text-lg">
                {{ pickLocalizedText(locale, section.column.text) }}
              </p>
            </template>

            <div v-if="section.column.cards.length" :class="cardsContainerClass(section.column.cards.length, section.column.align)">
              <div
                v-for="card in section.column.cards"
                :key="card.id"
                class="rounded-2xl border p-5 shadow-sm"
                :class="[cardWrapperClass(section.column.cards.length), cardClass(card)]"
...
const getColumnVerticalAlignClass = () => {
  ...
  if (column.type !== 'content') return 'flex min-h-full items-start'
  switch (column.verticalAlign) {
    case 'center':
      return 'flex min-h-full items-center'
    case 'end':
      return 'flex min-h-full items-end'
    default:
      return 'flex min-h-full items-start'
  }
}
