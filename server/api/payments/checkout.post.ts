...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.successUrl || !body?.cancelUrl || !Array.isArray(body.lineItems) || !body.lineItems.length) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Invalid checkout payload',
      message: 'Le paiement Stripe nécessite des URLs de retour et au moins une ligne de commande.'
    })
  }
