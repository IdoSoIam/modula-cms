<template>
  <div class="space-y-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <h3 class="font-semibold">{{ t('admin.productOptions.groupsTitle') }}</h3>
        <p class="mt-1 text-sm opacity-65">{{ t('admin.productOptions.groupsHelp') }}</p>
      </div>
      ...
      <div class="flex items-center justify-between gap-3">
        <button type="button" class="flex min-w-0 flex-1 items-center gap-3 text-left" @click="toggleGroup(group.id)">
          ...
          <span class="min-w-0">
            ...
            <span class="block text-xs opacity-60">{{ t('admin.productOptions.optionCount', { count: group.options.length }) }}</span>
          </span>
        </button>
        <button type="button" class="btn btn-square btn-sm btn-ghost text-error" :aria-label="t('admin.productOptions.removeGroup')" @click="removeGroup(groupIndex)">
        ...
        <div class="grid min-w-0 flex-1 gap-4 md:grid-cols-2">
          ...
          <label class="form-control gap-2">
            ...
            <select v-model="group.selectionMode" class="select select-bordered w-full">
              <option value="MULTIPLE">{{ t('admin.productOptions.selectionMultiple') }}</option>
              <option value="SINGLE">{{ t('admin.productOptions.selectionSingle') }}</option>
            </select>
          </label>
        ...
        <article v-for="(option, optionIndex) in group.options" :key="option.id" class="rounded-box border border-base-300 bg-base-100 p-4">
          <div class="flex items-start justify-between gap-3">
            <button type="button" class="flex min-w-0 flex-1 items-center gap-2 text-left" @click="toggleOption(group.id, option.id)">
              <Icon :name="collapsedOptionIds.has(optionCollapseKey(group.id, option.id)) ? 'mdi:chevron-right' : 'mdi:chevron-down'" size="18" class="shrink-0" />
            ...
            <button type="button" class="btn btn-square btn-sm btn-ghost text-error" :aria-label="t('admin.productOptions.removeOption')" @click="removeOption(groupIndex, optionIndex)">
          ...
          <div v-show="!collapsedOptionIds.has(optionCollapseKey(group.id, option.id))" class="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <label class="form-control gap-2">
              ...
              <select v-model="option.kind" class="select select-bordered w-full" @change="normalizeOptionKind(option)">
                <option value="SUPPLEMENT">{{ t('admin.productOptions.kindSupplement') }}</option>
                <option value="INSURANCE">{{ t('admin.productOptions.kindInsurance') }}</option>
                <option value="ACCESSORY">{{ t('admin.productOptions.kindAccessory') }}</option>
              </select>
            </label>
            <label v-if="option.kind === 'ACCESSORY'" class="form-control gap-2 md:col-span-2">
              <span class="label-text">{{ t('admin.productOptions.linkedProduct') }}</span>
              <select v-model.number="option.linkedProductId" class="select select-bordered w-full">
                <option :value="0">{{ t('admin.productOptions.selectProduct') }}</option>
                <option v-for="product in selectableProducts" :key="product.id" :value="product.id">
                  {{ product.name }} · {{ product.saleType === 'RENTAL' ? t('admin.productsPage.saleTypeRental') : t('admin.productsPage.saleTypeSale') }}
                </option>
              </select>
            </label>
            <label v-if="option.kind === 'INSURANCE'" class="form-control gap-2 md:col-span-2">
              ...
              <select v-model.number="option.billingDocumentId" class="select select-bordered w-full">
                <option :value="0">{{ t('admin.productOptions.selectDocument') }}</option>
                <option v-for="document in insuranceDocuments" :key="document.id" :value="document.id">{{ document.name }}</option>
              </select>
            </label>
            <label v-if="option.kind === 'ACCESSORY'" class="form-control gap-2">
              ...
              <select v-model="option.priceSource" class="select select-bordered w-full">
                <option value="LINKED_PRODUCT">{{ t('admin.productOptions.priceFromProduct') }}</option>
                <option value="CUSTOM">{{ t('admin.productOptions.customPrice') }}</option>
              </select>
            </label>
            ...
            <label v-if="option.kind !== 'ACCESSORY' || option.priceSource === 'CUSTOM'" class="form-control gap-2">
              ...
              <select v-model="option.pricingMode" class="select select-bordered w-full">
                <option value="FIXED">{{ t('admin.productOptions.pricingFixed') }}</option>
                <option value="RENTAL_DURATION">{{ t('admin.productOptions.pricingRentalDuration') }}</option>
              </select>
            </label>
            <template v-if="(option.kind !== 'ACCESSORY' || option.priceSource === 'CUSTOM') && option.pricingMode === 'RENTAL_DURATION'">
              ...
              <p class="self-end text-xs opacity-65">{{ t('admin.productOptions.durationPriceHelp') }}</p>
            </template>
            <label v-if="option.kind !== 'ACCESSORY'" class="form-control gap-2">
              ...
              <select v-model="option.quantityMode" class="select select-bordered w-full">
                <option value="PER_RESERVATION">{{ t('admin.productOptions.quantityReservation') }}</option>
                <option value="PER_PRODUCT_UNIT">{{ t('admin.productOptions.quantityProductUnit') }}</option>
              </select>
            </label>
            <label class="form-control gap-2">
            ...
            <label class="form-control gap-2">
              ...
              <input v-model.number="option.maxQuantity" type="number" :min="option.minQuantity" step="1" class="input input-bordered w-full" :placeholder="t('admin.productOptions.noMaximumQuantity')" />
            </label>
            <label class="label cursor-pointer justify-start gap-3 self-end">
            ...
            <label class="form-control gap-2">
              ...
              <input v-model.number="option.vatRate" type="number" min="0" max="100" step="0.01" class="input input-bordered w-full" :placeholder="t('admin.productOptions.vatInherited')" />
            </label>
            <label class="label cursor-pointer justify-start gap-3 self-end">
...
const emit = defineEmits<{ 'update:modelValue': [value: ProductOptionGroup[]] }>()
const { t } = useI18n()
...
const selectableProducts = computed(() => (props.products || []).filter(product => product.id !== props.excludedProductId))
...
function addOption(groupIndex: number) {
  groups.value = groups.value.map((group, index) => index === groupIndex
    ? { ...group, options: [...group.options, createOption(group.options.length)] }
    : group)
}
