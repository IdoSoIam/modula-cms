...
const createAdminNavigationItem = (
  ...
  requirements: Pick<
    AdminNavigationItem,
    "requiredModule" | "requiredAction" | "requiredSpecialPermission"
  > = {},
): AdminNavigationItem => ({
  id,
  labelKey,
  ...
  activePaths: activePaths.length
    ? Array.from(new Set(activePaths))
    : undefined,
  ...requirements,
});
  ...
  const filteredSections = sections
    .map((section) => ({
      ...
      items: section.items.filter((item) => {
        if (item.id === "content-news" || item.id === "customization-news")
          return newsEnabled;
        if (
          item.id === "content-events" ||
          item.id === "content-planning" ||
          item.id === "customization-events" ||
          item.id === "customization-planning" ||
