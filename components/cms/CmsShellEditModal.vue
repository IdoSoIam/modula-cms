              ...
              <div v-if="activeShellPanel === 'header'" class="space-y-4">
                ...
                <section class="collapse collapse-arrow rounded-2xl border border-base-300 bg-base-100" :class="isOpen('header-colors') ? 'collapse-open' : 'collapse-close'">
                  ...
                  <div class="collapse-content grid gap-4 lg:grid-cols-2">
                    <ThemeColorPicker v-model="model.settings.header.backgroundColor" label="Fond du header" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="base-100" />
                    <ThemeColorPicker v-model="model.settings.header.textColor" label="Texte du header" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="base-content" />
                    <ThemeColorPicker v-model="model.settings.header.navigationActiveBackgroundColor" label="Fond actif navigation" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="primary" />
                    <ThemeColorPicker v-model="model.settings.header.navigationActiveTextColor" label="Texte actif navigation" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="primary-content" />
                    <ThemeColorPicker v-model="model.settings.header.navigationHoverBackgroundColor" label="Fond survol navigation" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="base-200" />
                    <ThemeColorPicker v-model="model.settings.header.navigationHoverTextColor" label="Texte survol navigation" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="base-content" />
                    <ThemeColorPicker v-model="model.settings.header.submenuBackgroundColor" label="Fond sous-menu" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="base-100" />
                    <ThemeColorPicker v-model="model.settings.header.submenuTextColor" label="Texte sous-menu" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="base-content" />
                  </div>
                </section>
              </div>
                ...
                <section class="collapse collapse-arrow rounded-2xl border border-base-300 bg-base-100" :class="isOpen('nav-primary') ? 'collapse-open' : 'collapse-close'">
                  ...
                  <div class="collapse-content space-y-3">
                    ...
                    <article v-for="(item, index) in primaryNavigationItems" :key="item.navigationItemKey" class="collapse collapse-arrow rounded-2xl border border-base-300 bg-base-100" :class="isOpen(`nav-item-${item.navigationItemKey}`) ? 'collapse-open' : 'collapse-close'">
                      <button type="button" class="collapse-title w-full text-left font-medium" @click="toggleAccordion(`nav-item-${item.navigationItemKey}`)">{{ previewText(item.labels) || item.title || `Lien ${Number(index) + 1}` }}</button>
                      <div class="collapse-content space-y-4">
                  ...
                  <div class="collapse-content space-y-4">
                    <div class="grid gap-4 lg:grid-cols-2">
                      ...
                      <label class="form-control gap-2"><span class="label"><span class="label-text">Alignement des colonnes</span></span><select v-model="model.settings.footer.containerAlign" class="select select-bordered w-full"><option value="start">Début</option><option value="center">Centre</option><option value="between">Réparti</option></select></label>
                       <ThemeColorPicker v-model="model.settings.footer.backgroundColor" label="Fond du footer" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="neutral" />
                       <ThemeColorPicker v-model="model.settings.footer.textColor" label="Texte du footer" :allowed-tokens="themeColorTokens" :allow-custom="false" default-token="neutral-content" />
                    </div>
                ...
                <section class="collapse collapse-arrow rounded-2xl border border-base-300 bg-base-100" :class="isOpen('footer-social') ? 'collapse-open' : 'collapse-close'">
                  ...
                  <div class="collapse-content space-y-3">
                    ...
                    <article v-for="(link, index) in model.settings.socialLinks" :key="link.id" class="collapse collapse-arrow rounded-2xl border border-base-300 bg-base-100" :class="isOpen(`social-${link.id}`) ? 'collapse-open' : 'collapse-close'">
                      <button type="button" class="collapse-title w-full text-left font-medium" @click="toggleAccordion(`social-${link.id}`)">{{ socialLinkLabel(link, index) }}</button>
                      <div class="collapse-content space-y-4">
                        <div class="flex justify-end"><button type="button" class="btn btn-xs btn-outline btn-error" @click="model.settings.socialLinks.splice(Number(index), 1)">Supprimer</button></div>
                        <div class="grid gap-4 lg:grid-cols-2">
                ...
                <section class="collapse collapse-arrow rounded-2xl border border-base-300 bg-base-100" :class="isOpen('footer-columns') ? 'collapse-open' : 'collapse-close'">
                  ...
                  <div class="collapse-content space-y-3">
                    ...
                    <article v-for="(column, columnIndex) in model.settings.footer.columns" :key="column.id" class="collapse collapse-arrow rounded-2xl border border-base-300 bg-base-100" :class="isOpen(`footer-column-${column.id}`) ? 'collapse-open' : 'collapse-close'">
                      <button type="button" class="collapse-title w-full text-left font-medium" @click="toggleAccordion(`footer-column-${column.id}`)">{{ previewText(column.title) || `Colonne ${Number(columnIndex) + 1}` }}</button>
                      <div class="collapse-content space-y-4">
                        <div class="flex flex-wrap gap-2 justify-end">
                          ...
                          <button type="button" class="btn btn-xs btn-outline btn-error" @click="model.settings.footer.columns.splice(Number(columnIndex), 1)">Supprimer</button>
                        </div>
                        <div class="grid gap-4 lg:grid-cols-2">
                        ...
                        <article v-for="(block, blockIndex) in column.blocks" :key="block.id" class="collapse collapse-arrow rounded-xl border border-base-300 bg-base-200" :class="isOpen(`footer-block-${block.id}`) ? 'collapse-open' : 'collapse-close'">
                          <button type="button" class="collapse-title w-full text-left font-medium" @click="toggleAccordion(`footer-block-${block.id}`)">{{ footerBlockLabel(block.type as CmsFooterBlockType) }}</button>
                          <div class="collapse-content space-y-4">
                            <div class="flex flex-wrap gap-2 justify-end">
                              ...
                              <button type="button" class="btn btn-xs btn-outline btn-error" @click="column.blocks.splice(Number(blockIndex), 1)">Supprimer</button>
                            </div>
                            <div class="grid gap-4 lg:grid-cols-2">
...
const themeColorTokens = [...CMS_THEME_COLOR_TOKENS]
...
const previewText = (value?: Partial<Record<string, string>> | null) => value?.fr || value?.en || Object.values(value ?? {}).find(Boolean) || ''
const footerBlockLabel = (type: CmsFooterBlockType) => footerBlockLabels[type]
const socialLinkLabel = (link: CmsSocialLink, index: number | string) => previewText(link.label) || `RÃ©seau ${Number(index) + 1}`
...
const focusedNavigationItem = computed(() => {
  const focus = activeShellFocus.value
  if (focus?.kind !== 'navigation-item') return null
  return model.value?.navigation.find((item: any) => item.navigationItemKey === focus.key && item.menu === focus.menu) ?? null
})
const focusedFooterColumn = computed<CmsFooterColumn | null>(() => {
  const focus = activeShellFocus.value
  if (focus?.kind !== 'footer-column') return null
  return model.value?.settings.footer.columns.find((column: CmsFooterColumn) => column.id === focus.columnId) ?? null
})
const focusedFooterBlock = computed<CmsFooterBlock | null>(() => {
  const focus = activeShellFocus.value
  if (focus?.kind !== 'footer-block') return null
  const column = model.value?.settings.footer.columns.find((entry: CmsFooterColumn) => entry.id === focus.columnId)
  return column?.blocks.find((block: CmsFooterBlock) => block.id === focus.blockId) ?? null
})
const focusedSocialLink = computed<CmsSocialLink | null>(() => {
  const focus = activeShellFocus.value
  if (focus?.kind !== 'social-link') return null
  return model.value?.settings.socialLinks.find((link: CmsSocialLink) => link.id === focus.id) ?? null
})
...
const moveItem = <T,>(list: T[], index: number | string, direction: -1 | 1) => {
...
const moveNavigationItem = (menu: 'PRIMARY' | 'FOOTER', index: number | string, direction: -1 | 1) => {
