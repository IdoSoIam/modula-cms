...
function normalizeColumn(value: unknown): HomePageColumn | null {
  ...
  if (value.type === 'content') {
    return {
      ...
      badgeSize: (typeof value.badgeSize === 'string' ? value.badgeSize : 'sm') as Extract<HomePageColumn, { type: 'content' }>['badgeSize'],
      title: normalizeLocalizedText(value.title) || { fr: '', en: '' },
      titleSize: (typeof value.titleSize === 'string' ? value.titleSize : 'xl') as Extract<HomePageColumn, { type: 'content' }>['titleSize'],
      text: normalizeLocalizedText(value.text) || { fr: '', en: '' },
      textSize: (typeof value.textSize === 'string' ? value.textSize : 'md') as Extract<HomePageColumn, { type: 'content' }>['textSize'],
...
function normalizeHomePageContent(value: unknown, fallback: HomePageContent): HomePageContent {
  ...
  const normalizedHero = {
    ...
    badgeSize: typeof hero.badgeSize === 'string' ? hero.badgeSize as HomePageContent['hero']['badgeSize'] : 'sm',
    title: normalizeLocalizedText(hero.title) || fallback.hero.title,
    titleSize: typeof hero.titleSize === 'string' ? hero.titleSize as HomePageContent['hero']['titleSize'] : '2xl',
    text: normalizeLocalizedText(hero.text) || fallback.hero.text,
    textSize: typeof hero.textSize === 'string' ? hero.textSize as HomePageContent['hero']['textSize'] : 'lg',
