...
import { serializeProduct, serializeProductCategory, serializeProductLot } from '#modula/server/utils/shop'
...
export default defineEventHandler(async (event) => {
  ...
  const [products, productLots] = await Promise.all([
    view === 'lots'
      ? Promise.resolve([])
      : db.product.findMany({
          where: sharedWhere,
  ...
  return {
    ...
    productLots: productLots.map(serializeProductLot)
  }
})
