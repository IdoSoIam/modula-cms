<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-3xl font-bold">Paniers</h1>
      <button class="btn btn-primary" @click="openNew">
        <Icon name="mdi:plus" size="20" /> Nouveau panier
      </button>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="card-body">
          ...
          <div class="text-sm mt-2">
            <div class="flex justify-between">
              ...
              <span>{{ b.items.length }} légume(s)</span>
            </div>
            <div class="flex justify-between">
              <span class="opacity-70">Prix calculé :</span>
            ...
            <div class="flex justify-between">
              <span class="opacity-70">Disponibles :</span>
          ...
          <div class="card-actions justify-end mt-2">
            <button class="btn btn-ghost btn-sm" @click="openEdit(b)">
              <Icon name="mdi:pencil" size="16" /> Modifier
            </button>
      ...
      <div v-if="!baskets?.length" class="md:col-span-2 text-center opacity-60 py-8">
        Aucun panier. Créez-en un pour proposer une réservation aux clients.
      </div>
    </div>
    ...
    <dialog ref="dlg" class="modal">
      <div class="modal-box max-w-3xl">
        <h3 class="font-bold text-lg mb-4">
          {{ editing.id ? 'Modifier le panier' : 'Nouveau panier' }}
        </h3>
        ...
        <div class="space-y-2">
          ...
          >
            <select v-model.number="it.vegetableId" class="select select-bordered flex-1">
              <option :value="0" disabled>— Choisir un légume —</option>
            ...
            <span class="text-sm opacity-70 w-20">
              {{ vegetables?.find(v => v.id === it.vegetableId)?.unit === 'KG' ? 'kg' : 'pièce(s)' }}
            </span>
          ...
          <button class="btn btn-sm btn-outline" @click="addItem">
            <Icon name="mdi:plus" size="16" /> Ajouter un légume
          </button>
        </div>
